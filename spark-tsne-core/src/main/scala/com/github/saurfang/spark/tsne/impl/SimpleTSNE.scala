  ...
  def tsne(
    ...
    while(iteration <= maxIterations) {
      ...
      if (iteration % 100 == 0) {
        ...
        val file1 = new File(s"logs/neighbor_default_1000iterations.txt")
        val bw = new BufferedWriter(new FileWriter(file1,true))
        bw.write(s"the${iteration} is : ${neighbor} \n")
        bw.close()
              }
      ...
      TSNEHelper.update(Y, dY, iY, iteration, tsneParam)
...
//    val Y: DenseMatrix[Double] = DenseMatrix.rand(n, noDims, Rand.gaussian(0, 1))
//    val iY = DenseMatrix.zeros[Double](n, noDims)
//    val gains = DenseMatrix.rand(n, noDims, Rand.gaussian(0, 1))
//
//    // approximate p_{j|i}
//    val p_ji = X2P(input, 1e-5, perplexity)
//    val P = TSNEHelper.computeP(p_ji, n).glom().cache()
...
//    var iteration = 1
//    while(iteration <= maxIterations) {
//      val bcY = P.context.broadcast(gains)
//
//      val numerator = P.map{ arr => TSNEGradient.computeNumerator(bcY.value, arr.map(_._1): _*) }.cache()
//      val bcNumerator = P.context.broadcast({
//        numerator.treeAggregate(0.0)(seqOp = (x, v) => x + sum(v), combOp = _ + _)
...
//
//      if (iteration % 10 == 0) {
//        val bcY1 = P.context.broadcast(Y)
//        val numerator1 = P.map{ arr => TSNEGradient.computeNumerator(bcY1.value, arr.map(_._1): _*) }.cache()
//        val bcNumerator1 = P.context.broadcast({
//          numerator1.treeAggregate(0.0)(seqOp = (x, v) => x + sum(v), combOp = _ + _)
//        })
//        val Q = numerator1.map(arr => arr / bcNumerator1.value)
//        val neighbor = P.zip(Q).map(arr => TSNEHelper.NeiborhoodPreserveMatrix(arr._1, arr._2, K)).sum() / (n * 1.0)
//        val file1 = new File(s"logs/neighbor_default_Nagtest.txt")
//        val bw = new BufferedWriter(new FileWriter(file1,true))
//        bw.write(s"the${iteration} is : ${neighbor} \n")
//        bw.close()
//      }
